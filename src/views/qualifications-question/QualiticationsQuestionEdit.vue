<template>
    ...
    <v-main>
        ...
        <v-btn 
            flat :disabled="mode=='edit'"
            rounded="lg" class="submit mx-2"
        ...
        >ยกเลิก
            ...
            <card-header title="แก้ไขคำถามตรวจสอบเงื่อนไขคุณสมบัติผู้กู้ยืม"/>
            <v-container>
                <v-row justify="space-between" class="mx-8">
                    ...
                    auto-grow rows="1"
                    clearable placeholder="คำถาม"
                    label="คำถาม" variant="underlined"
                    rounded="xl" v-model="question"
            ...
            <v-card-actions class="mx-8 my-2">
                <v-btn 
                    flat rounded="lg" 
                    class="submit mx-2" v-if="mode=='create'"
                    @click="switchMode"
                >เลือกคำตอบที่ผ่านเงื่อนไข
                </v-btn>
                <v-btn 
                    variant="outlined" rounded="lg" 
                    class="info mx-2" v-if="mode=='edit'"
                    @click="mode='create'"
                >เสร็จสิ้น
                </v-btn>
            </v-card-actions>
...
export default{
    ...
    data() {
        return {
            ...
            items: [
                ...
                {
                    title: 'แก้ไขคำถามตรวจสอบเงื่อนไขคุณสมบัติผู้กู้ยืม',
                    disabled: true,
    ...
    methods: {
        getQuestion() {
            ...
            axios.get('qualification-question/qualification-questions/'+pathSplit[3])
            .then(response => {
                this.question = response.data.question
                this.selectedItem = response.data.correct_answer.split('"')[1]
                this.answers = JSON.parse(response.data.choice)
            }).catch(error => {
                console.log(error)
            })
        },
        ...
        onSubmit() {
            ...
            if (this.question=="" || answer_null==true) {}
            if (dup==true && this.question!=null && answer_null==false) {
                this.$swal({
                        title: 'แก้ไขคำถามไม่สำเร็จ',
                        showCloseButton: true,
                        text: "ไม่สามารถสร้างคำตอบซ้ำได้ กรุณาแก้ไขคำตอบ",
                        confirmButtonText: "ตกลง",
                        confirmButtonColor: "#0AAE75",
                        icon: "error",
                        backdrop: false
            ...
            if (this.question!="" && (dup==false) && this.selectedItem.length!=0 && (answer_null==false)) {
                ...
                const pathSplit = this.$route.path.split('/')
                axios.put('qualification-question/qualification-questions/'+pathSplit[3]+'/', formData)
                .then(response => {
                    this.$swal({
                        ...
                        if (result.isConfirmed || result.isClosed) {this.$router.push('/admin/qualifications-question/')}
                    })
                }).catch(error => {
                    console.log(error)
                })
